...
for temp_file in imput_file_path_list [0:10]:  ###remove [0:10] for full file run
    ...
    out_folder_path = "/scratch/christe7/BB485/Week06/mafft_out/"

    ## Create a new file path pointing to the output directory (this is how we tell mafft what to name the output)
    new_file_path = full_path_to_file.replace(imput_folder_path, out_folder_path)
...
print(f"Numer of unknown trees: {top_list.count('Unknown')}")
